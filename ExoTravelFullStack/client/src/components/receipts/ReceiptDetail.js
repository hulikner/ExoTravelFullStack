...
export const ReceiptDetail = () => {
  ...
  const formattedDepartureDate = receipt?.departureDate && epochDateConverter(receipt.departureDate, "MM/dd/yyy");
  const formattedReturnDate = receipt?.returnDate && epochDateConverter(receipt.returnDate, "MM/dd/yyy");
  ...
  const handleDelete = () => {
    setIsLoading(true);
    deleteReceipt(receiptId).then(() => navigate("/logs"));
  };
    ...
    <>
      ...
      <div className="receipt-departure-return">
        <div className="receipt-departureToReturn">
          <span className="receipt-detail">Departure: {formattedDepartureDate} @ 8:00 AM to The Citadel</span>
          <br />
          <span className="receipt-detail">Gate Number: 2C </span>
          <br />
          <span className="receipt-detail">Shuttle Number: {receipt.departureDate}</span>
          <br />
        ...
        <div className="receipt-departureToReturn">
          <span className="receipt-detail">
            Departure: {formattedDepartureDate} @ 12:00 PM to {receipt.exoPlanetId?.name}
          </span>
          <br />
        ...
        <div className="receipt-departureToReturn">
          <span className="receipt-detail">Return: {formattedReturnDate} @ 8:00 AM to The Citadel</span>
          <br />
          <span className="receipt-detail">Gate Number: X-4 </span>
          <br />
          <span className="receipt-detail">Shuttle Number: R2-D2</span>
        ...
        <div className="receipt-departureToReturn">
          <span className="receipt-detail">Return: {formattedDepartureDate} @ 8:00 AM to Origin</span>
          <br />
          <span className="receipt-detail">Gate Number: 2C </span>
          <br />
          <span className="receipt-detail">Shuttle Number: {receipt.returnDate}</span>
          <br />
          <span className="receipt-detail">Flight Duration: 2 Hours</span>
          <br />
        </div>
